...
st.sidebar.caption(
    ...
    "- リスク期内の積算降水量が「高リスク上限」未満 → **高リスク（乾燥）**\n"
    "- 「高リスク上限」以上「中リスク上限」未満 → **中リスク**\n"
    "- 「中リスク上限」以上 → **低リスク（湿潤）**"
)
threshold_high = st.sidebar.number_input(
    "高リスク上限 (mm) ← これ未満で高リスク",
    value=THRESHOLD_HIGH_DEFAULT,
    help=f"リスク期の積算降水量がこの値未満の場合「高リスク」と判定します（デフォルト: {THRESHOLD_HIGH_DEFAULT}mm）"
)
threshold_med = st.sidebar.number_input(
    "中リスク上限 (mm) ← これ未満で中リスク",
    value=THRESHOLD_MED_DEFAULT,
    help=f"積算降水量がこの値未満（かつ高リスク上限以上）の場合「中リスク」と判定します（デフォルト: {THRESHOLD_MED_DEFAULT}mm）"
)
...
def calculate_scab_risk(p_date, weather_df, b_temp, g_start, g_end, t_high, t_med):
    ...
    # 降水量が少ないほど高リスク（乾燥条件でそうか病感染拡大）
    if total_precip < t_high:
        risk_l, risk_c, risk_v = "高 (High)", "#FF4B4B", 2
    elif total_precip < t_med:
        risk_l, risk_c, risk_v = "中 (Medium)", "#FFA500", 1
    else:
        risk_l, risk_c, risk_v = "低 (Low)", "#0068C9", 0
...
def plot_period_analysis(results_df, t_high, t_med):
    ...
    # ===== [修正1] グラフ内ラベルも明確化 =====
    ax.text(p_dates_num[0], t_high + 2, f"↑ 高リスク境界 {t_high}mm（以下で高リスク）", color="#FF4B4B", fontsize=9)
    ax.text(p_dates_num[0], t_med + 2,  f"↑ 中リスク境界 {t_med}mm（以下で中リスク）",  color="#FFA500",  fontsize=9)
    ...
    high_patch = mpatches.Patch(color="#FF4B4B", label="高リスク (High)：乾燥")
    med_patch  = mpatches.Patch(color="#FFA500", label="中リスク (Medium)")
    low_patch  = mpatches.Patch(color="#0068C9", label="低リスク (Low)：湿潤")
...
def build_csv(results_df: pd.DataFrame) -> bytes:
    ...
    show_df = results_df[results_df['status'] == '判定完了'][
        ['planting_date', 'start_date_w', 'end_date_w', 'reached_end', 'total_precip', 'risk_level']
    ].copy()
    ...
    show_df.columns = [
        '植え付け日', 'リスク期開始日', 'リスク期終了日',
        'GDD終了閾値到達', 'リスク期積算降水量(mm)', 'リスクレベル'
    ]
...
if st.sidebar.button("▶ リスク分析を実行", type="primary"):
    ...
    if analysis_mode == "単一日の判定":
        ...
        <div style="background-color:{res['risk_color']}18; border-left:5px solid {res['risk_color']}; padding:15px; border-radius:5px; margin-top:10px;">
            ...
            <p style="margin-top:8px; font-size:15px;">期間中の積算降水量: {res['total_precip']:.1f} mm
            （高リスク上限: {threshold_high}mm ／ 中リスク上限: {threshold_med}mm）</p>
        </div>
        ...
        bar = st.progress(0)
        update_interval = max(1, len(date_list) // 20)  # 約5%刻みで更新
        ...
        with col_info:
            ...
            if not completed.empty:
                high_count = (completed['risk_value'] == 2).sum()
                med_count  = (completed['risk_value'] == 1).sum()
                low_count  = (completed['risk_value'] == 0).sum()
                st.markdown(
        ...
        with st.expander("📋 分析結果の詳細データテーブル"):
            show_df = results_df[results_df['status'] == '判定完了'][
                ['planting_date', 'total_precip', 'risk_level', 'start_date_w', 'end_date_w', 'reached_end']
            ].copy()
            show_df['planting_date'] = pd.to_datetime(show_df['planting_date']).dt.strftime('%Y/%m/%d')
            show_df['start_date_w']  = pd.to_datetime(show_df['start_date_w']).dt.strftime('%Y/%m/%d')
            show_df['end_date_w']    = pd.to_datetime(show_df['end_date_w']).dt.strftime('%Y/%m/%d')
            show_df['reached_end']   = show_df['reached_end'].map({True: '到達', False: '未到達'})
            show_df['total_precip']  = show_df['total_precip'].round(1)
            show_df.columns = ['植え付け日', 'リスク期 降水量(mm)', 'リスクレベル', 'リスク期 開始日', 'リスク期 終了日', 'GDD終了閾値到達']
